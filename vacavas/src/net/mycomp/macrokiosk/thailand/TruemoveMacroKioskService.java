...
@Service("truemoveMacroKioskService")
public class TruemoveMacroKioskService extends  AbstractMacroKioskMTMessage{

	private static final Logger logger = Logger.getLogger(TruemoveMacroKioskService.class);
	...
	@Value("${thialand.truemove.mo.send.url}")
	private String moUrl;
	
	@Value("${thialand.truemove.mt.send.url}")
		...
		if(deliveryNotification.getNotificationType().equalsIgnoreCase(ThiaConstant.MT_BIILABLE_MESSAGE)){	
			if(deliveryNotification!=null&&deliveryNotification.getStatus()!=null&&
					(deliveryNotification.getStatus().contains("external:DELIVRD")
							...
							||deliveryNotification.getStatus().contains("Sent_Delivered: 000")	
							)){
